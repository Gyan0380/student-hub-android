...
import com.google.firebase.firestore.Query; // ADDED
...
public class NotificationsActivity extends AppCompatActivity {
    ...
    private void listen(TextView empty) {
        ...
        Query q = db.collection("Notifications")
                .whereIn(
                        "toUid",
                        java.util.Arrays.asList("all", uid)
                )
                .limit(100);
        ...
        listener = q.addSnapshotListener((snap, e) -> {
            if (e != null) {
                Toast.makeText(
                        this,
                        "Notifications load nahi hui: " + e.getMessage(),
                        Toast.LENGTH_LONG
                ).show();
                return;
            }
            ...
            if (snap != null) {
                for (DocumentSnapshot d : snap.getDocuments()) {
                    AppNotification n =
                            d.toObject(AppNotification.class);
