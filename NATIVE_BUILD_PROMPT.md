...
- **Build fix (this session)**: `HomeViewModel.kt` referenced `data.model.RoomIds`
  (`RoomIds.GLOBAL/ANONYMOUS/ADMIN_ROOM/slugifyClassName()`) from Step 4 below, but that
  file was never actually created — caused a real `compileDebugKotlin` failure
  ("Unresolved reference: RoomIds") in CI. Added `data/model/RoomIds.kt` with the three
  constants (`"global"`, `"anonymous"`, `"admin-room"`, matching the roomId convention
  documented under step 3's data model) and a `slugifyClassName` (lowercase,
  non-alphanumeric runs → single `-`, trimmed) matching `CLASS_ROOM_OPTIONS` in
  `User.kt`. Also added `kotlinx-coroutines-play-services` to `app/build.gradle`, which
  every repo using `Task<T>.await()` needed but never had. Cross-checked every internal
  `com.studenthub.app.*` import in the project against its declaration — no other
